 ...
 */
var AssemblerInfoProvider = function () {
  function AssemblerInfoProvider() {
    var fglob = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'build/contracts/**/*.json';
    (0, _classCallCheck3.default)(this, AssemblerInfoProvider);

    this.artifactsGlob = fglob;
  }

  (0, _createClass3.default)(AssemblerInfoProvider, [{
     ...
     * @return {*}
     */
    value: function getContractDataIfExists(bytecode) {
      var _this = this;

      if (!bytecode.startsWith('0x')) {
        throw new Error('0x hex prefix missing: ' + bytecode);
      }
      return this.contractsData.datas.find(function (contractDataCandidate) {
        var bytecodeRegex = _this.bytecodeToBytecodeRegex(contractDataCandidate.bytecode);
        var runtimeBytecodeRegex = _this.bytecodeToBytecodeRegex(contractDataCandidate.runtimeBytecode);
    ...
    get: function get() {
      if (!this._contractsData) {
        var artifactFileNames = _glob2.default.sync(this.artifactsGlob, { absolute: true });
        ...
        var _sourceCodes = sources.map(function (source) {
          if (!source.source) {
  ...
  return AssemblerInfoProvider;
}();

exports.default = AssemblerInfoProvider;
